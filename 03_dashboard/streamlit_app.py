...
from pipeline_runner import BASE_FEATURES, load_project_config, predict_manual_case, run_pipeline
...
def _pick_selected_patient(latest_status: pd.DataFrame, default_patient: str | None) -> str | None:
    ...
    return st.sidebar.selectbox("Patient focus", choices, index=index, key="selected_patient")
...
with st.container(border=True):
    st.subheader("Quick Patient Check")
    st.caption("Enter the core features directly to get an instant Normal or Dangerous Situation prediction, then email the result immediately.")

    with st.form("manual_patient_check_form"):
        ...
        for idx, feature in enumerate(BASE_FEATURES[:4]):
            manual_feature_values[feature] = feature_cols_1[idx].number_input(
                feature,
        ...
        for idx, feature in enumerate(BASE_FEATURES[4:]):
            manual_feature_values[feature] = feature_cols_2[idx].number_input(
                feature,
                value=float(default_values[feature]),
                step=0.1,
                key=f"manual_{feature}",
        ...
        action_cols = st.columns([1.2, 1.2, 1, 1])
        manual_patient_id = action_cols[0].text_input("Case label", value="manual_case")
        # manual_email = action_cols[1].text_input(
        #     "Email for this result",
        #     value=st.session_state.get("recipient_email", ""),
        # )
        auto_email_dangerous = action_cols[2].checkbox("Auto-email dangerous result", value=True)
        manual_history_len = action_cols[3].number_input("History rows", min_value=6, max_value=12, value=6, step=1)

        manual_submit = st.form_submit_button("Predict this case", width="stretch")
